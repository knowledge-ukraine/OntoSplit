...
def get_classes(p_dict, p_key, output, title, uncertain, counter):
    def make_individual(item, title, key, output, counter):
        ...
        if len(new_entity_id) > 0:
            ...
            if new_entity_id not in output["entities"]:
                new_individual = OntoEntity(id="n_ind_" + new_entity_id,
                                            ...
                                            label=" ".join(new_name.split()[:7]).replace(" & ", " and ").replace(
                                                           ...
                                                           ">", "more than"),
                                            language="en",
                                            assertions={"relate_to_article": ["n_ind_"+title_id]})
                ...
                if title_id not in output["entities"][new_entity_id].assertions["relate_to_article"]:
                    output["entities"][new_entity_id].assertions["relate_to_article"].append("n_ind_"+title_id)
    ...
    if title not in output["classes"]:
        ...
        new_title_individual = OntoEntity(id="n_ind_"+new_title_entity_id, of_classes="articles_names",
                                          assertions={"relate_to_article": ["n_ind_"+new_title_entity_id]},
...
if __name__ == "__main__":
    ...
    all_words = set()
    ch_len = int(len(input_files_names) / int(settings["split to"]) + 1)
    ...
    for n, chunk in enumerate(input_files_names_ch):
        ...
        tau_0 = time.time()
        owl_text = make_owl(onto_objects, path=settings["owl path"], threads_n=settings["threads_n"])
        print(time.time() - tau_0, " sec")
        print("Saving OWL file")
        tau_0 = time.time()
        save_owl(str(settings["res dir"]) + "/" + str(settings["base file name"]) + str(n+1) + ".owl",
                 text_owl=owl_text)
        print(time.time() - tau_0, " sec")
        print("Process is complete")
        print()
    print(len(all_words), " words found")
    f = open(str(settings["res dir"]) + "/all_words.json", "w", encoding="utf-8")
