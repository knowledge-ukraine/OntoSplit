...
class QueryExec:
    ...
    def one_ont_parallel_quering(self, query, ont, mult=1):
        global is_running
        is_running = True
        s_time = time.time()
        out_data = list()
        if query is not None:
            pr_pool = [Thread(target=self.multiontology_query, name=str(ont) + "_" + str(n),
                              args=[query, ont, out_data], daemon=True)
    ...
    def one_ont_seq_quering(self, query, ont, mult=1):
        s_time = time.time()
        out_data = list()
        if query is not None:
            for n in range(int(mult)):
                out_data += self.single_query(query, ont)
        ex_time = time.time() - s_time
        return out_data, ex_time
...
if __name__ == "__main__":
    ...
    ontology_settings_file = "quering_settings.json"
    ...
    queries_mult = int(ontology_settings["queries_multiplication"])
    ...
    query_exec = QueryExec(jena_endpoint=ontology_settings["jena_endpoint"],
                           ...
                           ontologies=[ontology_settings["ontology"]])
    results = dict()
    ...
    for n_q, query in enumerate(test_queries):
        ...
        for i in range(queries_repeating):
            if ontology_settings["parallel"]:
                res, t = query_exec.one_ont_parallel_quering(query, query_exec.ontologies[0], mult=queries_mult)
            else:
                res, t = query_exec.one_ont_seq_quering(query, mult=queries_mult)
